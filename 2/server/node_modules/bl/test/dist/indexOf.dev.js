...
tape('indexOf takes a Uint8Array search', function (t) {
  var bl = new BufferList(['abcdefg', 'abcdefg']);
  var search = new Uint8Array([102, 103, 97, 98, 99]); // fgabc
...
tape('indexOf the entire nodejs10 buffer test suite', function (t) {
  ...
  var bufBc = Buffer.from('bc');
  var bufF = Buffer.from('f');
  var bufZ = Buffer.from('z');
  ...
  var asciiString = Buffer.from('somethingnotatallsinisterwhichalsoworks');
  t.equal(-1, asciiString.indexOf('\x2061'));
  t.equal(3, asciiString.indexOf('eth', 0)); // Search in string containing many non-ASCII chars.
