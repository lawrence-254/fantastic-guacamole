...
var encodings = ('hex utf8 utf-8 ascii binary base64' + (process.browser ? '' : ' ucs2 ucs-2 utf16le utf-16le')).split(' ');
...
tape('test readFloatLE / readFloatBE', function (t) {
  ...
  bl.append(buf2);
  bl.append(buf3);
  var canonical = Buffer.concat([buf1, buf2, buf3]);
...
tape('test toString encoding', function (t) {
  var bl = new BufferList();
  var b = Buffer.from('abcdefghij\xff\x00');
  bl.append(Buffer.from('abcd'));
  bl.append(Buffer.from('efg'));
  bl.append(Buffer.from('hi'));
  bl.append(Buffer.from('j'));
...
!process.browser && tape('test stream', function (t) {
  ...
  var bl = new BufferList(function (err, buf) {
    ...
    bl.pipe(fs.createWriteStream('/tmp/bl_test_rnd_out.dat')).on('close', function () {
      var rndhash = crypto.createHash('md5').update(random).digest('hex');
...
tape('instantiation with Buffer', function (t) {
  ...
  var buf2 = crypto.randomBytes(1024);
  var b = BufferList(buf);
...
tape('copy in an interval', function (t) {
  ...
  var b = BufferList(rnd); // put the random bytes there

  var actual = Buffer.alloc(3);
  var expected = Buffer.alloc(3);
...
!process.browser && tape('destroy with pipe after read end', function (t) {
  t.plan(2);
  var bl = new BufferList();
  fs.createReadStream(path.join(__dirname, '/test.js')).on('end', onEnd).pipe(bl);
