...
function (_EE) {
  ...
  function Parser(opt) {
    ...
    _this.filter = typeof opt.filter === 'function' ? opt.filter : noop; // Unlike gzip, brotli doesn't have any magic bytes to identify it
    // Users need to explicitly tell us they're extracting a brotli file
    // Or we infer from the file extension

    var isTBR = opt.file && (opt.file.endsWith('.tar.br') || opt.file.endsWith('.tbr')); // if it's a tbr file it MIGHT be brotli, but we don't know until
    // we look at it and verify it's not a valid tar file.

    _this.brotli = !opt.gzip && opt.brotli !== undefined ? opt.brotli : isTBR ? undefined : false; // have to set this so that streams are ok piping into it
  ...
  _createClass(Parser, [{
    ...
    value: function write(chunk) {
      ...
      var needSniff = this[UNZIP] === null || this.brotli === undefined && this[UNZIP] === false;
      ...
      if (needSniff && chunk) {
        ...
        var maybeBrotli = this.brotli === undefined;
