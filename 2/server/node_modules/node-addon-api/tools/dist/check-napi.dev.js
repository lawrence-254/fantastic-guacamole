'use strict'; // Descend into a directory structure and, for each file matching *.node, output
// based on the imports found in the file whether it's an N-API module or not.

var fs = require('fs');

var path = require('path'); // Read the output of the command, break it into lines, and use the reducer to
// decide whether the file is an N-API module or not.


function checkFile(file, command, argv, reducer) {
  var child = require('child_process').spawn(command, argv, {
...
function checkFileUNIX(file) {
  checkFile(file, 'nm', ['-a', file], function (soFar, line) {
    if (soFar === undefined) {
...
function checkFileWin32(file) {
  checkFile(file, 'dumpbin', ['/imports', file], function (soFar, line) {
    if (soFar === undefined) {
...
function recurse(top) {
  fs.readdir(top, function (error, items) {
    if (error) {
      ...
      fs.stat(item, function (item) {
        return function (error, stats) {
          ...
          if (stats.isDirectory()) {
            recurse(item);
          } else if (/[.]node$/.test(item) && // Explicitly ignore files called 'nothing.node' because they are
