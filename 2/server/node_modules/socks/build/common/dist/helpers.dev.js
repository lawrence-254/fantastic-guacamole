...
function ipv4ToInt32(ip) {
  var address = new ip_address_1.Address4(ip); // Convert the IPv4 address parts to an integer

  return address.toArray().reduce(function (acc, part) {
    return (acc << 8) + part;
  }, 0);
}
...
function int32ToIpv4(int32) {
  ...
  var octet1 = int32 >>> 24 & 0xff;
  var octet2 = int32 >>> 16 & 0xff;
  var octet3 = int32 >>> 8 & 0xff;
  var octet4 = int32 & 0xff; // Combine the octets into a string in IPv4 format
...
function ipToBuffer(ip) {
  if (net.isIPv4(ip)) {
    ...
    var address = new ip_address_1.Address4(ip);
    return Buffer.from(address.toArray());
  } else if (net.isIPv6(ip)) {
    // Handle IPv6 addresses
    var _address = new ip_address_1.Address6(ip);

    return Buffer.from(_address.canonicalForm().split(':').map(function (segment) {
